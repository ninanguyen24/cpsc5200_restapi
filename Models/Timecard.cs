...
namespace restapi.Models
{
    public class Timecard
    {
        ...
        //Replace line
        public TimecardLine ReplaceLine(string oldId, DocumentLine newTimecardLine)
        {
            TimecardLine newLine = null;

            for (int i = 0; i < Lines.Count; i++){
                if(Lines[i].UniqueIdentifier.ToString() == oldId){
                    newLine = Lines[i];
                    Lines[i].Week = newTimecardLine.Week;
                    Lines[i].Year = newTimecardLine.Year;
                    Lines[i].Day = newTimecardLine.Day;
                    Lines[i].Hours = newTimecardLine.Hours;
                    Lines[i].Project = newTimecardLine.Project;
                    break;
                }
            }
        ...
        //Added for Patch line
        public TimecardLine PatchLine(string oldId, DocumentLine newTimecardLine)
        {
            TimecardLine newLine = null;
